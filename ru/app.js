import { ASPECTS, PRONOUNS, TENSES, cellIndex, conjugate, classify, infinitiveLabel, isCorrect, withPronoun } from "./conjugate.js";
...
let verb;
let answers = [];
// 過去と仮定法だけ、女性形にした別解 (читал / читала)。それ以外は answers と同じ文字列
let femAnswers = [];
...
function renderGloss() {
  $("gloss").innerHTML = `<span class="chip">${classify(verb).label}</span>` + [
    `<i>${verb.aspect}</i> ${ASPECTS[verb.aspect]}`,
...
function answerHtml(p, form, fem, tense) {
  const s = withPronoun(p, form, tense);
  const pr = s.slice(0, -form.length); // "я " / "он/она " / 命令法では ""
  // 女性形で答えたなら主語も女性に絞る (он/она → она)
  return `<span class="pr">${fem ? pr.replace(/^он\//, "") : pr}</span>${form}`;
}
...
const byKind = Object.groupBy(
  verbs
    ...
    .sort(([, a], [, b]) =>
      ...
      a.infinitive.localeCompare(b.infinitive, "ru")),
  ([, v]) => classify(v).label,
);
...
const PINS_KEY = "spryazhenie.pins";
